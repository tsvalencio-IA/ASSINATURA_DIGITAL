;(function () {
  ...
  async function loadPdf(file) {
    ...
    state.pdfDocProxy = await W.pdfjsLib.getDocument({ data: buf.slice(0) }).promise;
  ...
  function fillPageSelect() {
    ...
    $('btnPaginaAnterior').disabled = state.type !== 'pdf' || state.pdfPage <= 1;
    $('btnPaginaProxima').disabled = state.type !== 'pdf' || state.pdfPage >= total;
  ...
  function updatePreviewMeta() {
    if (state.type === 'pdf') {
      $('previewMeta').textContent = `PDF com ${state.pdfPages} página(s). Assinatura na página ${state.pdfPage}.`;
  ...
  function currentPlacementForPdf(pdfPage) {
    ...
    const modoFim = $('assinadorModo')?.value === 'fim';
